...
| **Decline accuracy** — share of repeats reaching the expected outcome, and the expected reason when declined            | 87% frozen / 85% live — 2026-09-09, 35 cases × 5 repeats each | 80% provisional — see the open question        |
...
| An unattended nightly run spends money forever with nobody watching the bill.                                                                                | Quiet cost, discovered on an invoice.                                                                                                               | Cost recorded and shown per run (R10). No ceiling this pass — a deliberate choice; revisit if the number surprises anyone. **Measured 2026-09-09: ~$0.046 a run, so the nightly is ~$17/year** — no ceiling needed. Read that as an upper bound: both runs were priced with `cachedInputTokens` stuck at zero (see `ai-features.md`), so every token was charged at the fresh rate. |
...
- [ ] **Target for decline accuracy: the baseline now exists; the number still
      needs an owner.** Two full runs on 2026-09-09 — 35 cases × 5 repeats, one
      per corpus — came in at **87% frozen (153/175)** and **85% live
      (149/175)**, so `EVAL_THRESHOLD.declineAccuracy` is shipping at **0.8**:
      under both observed figures with 5–7pp of headroom, which is enough to
      survive ordinary run-to-run variance without a green board meaning
      nothing. That is a defensible provisional number, not a target anybody has
      agreed to — **confirm who sets it.** Two things to weigh when they do.
      The 2pp frozen-vs-live gap is corpus drift, not noise (three cases miss on
      the live article table and land on the frozen file), so a single target
      spanning both series is already a compromise. And ~3pp of the shortfall is
      one case, `feature-suggestion`, which is 0/5 on _both_ corpora and looks
      like a wrong expectation rather than a model failure — fixing it moves the
      baseline before the target is ever tested.
