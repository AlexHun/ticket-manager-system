...
| One eval run uses up the $1 daily cap                                           | Later visitors that day see AI features refuse  | Running evals counts against the cap (R8); see open question on evals   |
...
- [ ] **Assumed:** admin configuration is read-only in a demo session (R5). That
      protects the owner's knowledge-base and automation edits from a nightly reset
      that would have to undo them. Confirm with the owner; the alternative is to
      reset configuration too.
- [ ] **Assumed:** 5 demo sessions per IP per hour (R9). Confirm with the owner.
- [ ] **Assumed:** the reset and the AI-cap day both roll over at 00:00 UTC.
      Confirm with the owner.
- [ ] Should a demo session be able to start an eval run at all, given one run
      could use most of the $1 cap? _Blocks R8's final shape._ Needs the owner.
- [ ] Target numbers for demo sessions per week and the open-a-ticket share. Needs
      the owner.
- [ ] **Assumed:** the cap counts only AI calls a demo session starts. Background
