...
- **`pre-push` — `bun run typecheck` then the `@ticket/api` suite, under a minute.** The API suite plus a typecheck the commit hook may never have run (a `--no-verify`, another tool's commit, a rebase); measured 2026-09-24 on a Windows dev machine, warm, at typecheck ~16s and `@ticket/api` ~33s. **The web suite is in neither hook.** It is jsdom-bound, Windows runs it ~4x slower than `ubuntu-latest` (see [testing.md](testing.md)), and it cost ~4m54s of every push; CI runs it on every PR, and CLAUDE.md makes a green PR the finish line, so CI owns it. Run `bun run test` by hand when a change is mostly web and you want the answer before CI does. E2E is in neither hook either: it needs Postgres, a migrated `ticket_manager_test` and two servers on alt ports (`SCRIPTS.md`), and CI's `e2e` job owns it.
