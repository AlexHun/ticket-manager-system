...
Grounding material for AI-drafted replies. Nothing reads this file yet — it is
the fixture the Phase 5 draft-reply work (`implementation-plan.md`) will be
built and tested against, and the shape a real knowledge base should arrive in.
...
5. **Article ids are stable and never reused.** They are what
   `citedArticleIds` will refer to. Ids do not encode the category — an article
...
Two consequences worth stating before someone builds on this:
