/**
 * The showcase: 100 demo tickets and their email threads — dev databases, and
 * production only while it is a showcase with no real customer mail (see
 * SCRIPTS.md).
 *
 * Two callers, one implementation (#323). `prisma/seed-tickets.ts` is a thin
 * command line over `seedShowcase`, and `jobs/demo-reset.ts` runs the `reset`
 * mode every night at 00:00 UTC. It lived in the script until the nightly
 * needed it, and it moved rather than being copied so the two cannot drift:
 * what `--reset` puts back by hand is exactly what the night puts back.
 *
...
function buildRows(assignees: Assignee[], now: number): TicketRow[] {
  ...
  const rows = TICKETS.map((ticket, i) => {
    ...
    // "Today at 08:00" is still ahead of a run at midnight, which is when the
    // nightly reset runs. A ticket from the future sorts above everything and
    // drops out of every window that slices on `createdAt < to`, so it moves
    // to the day before.
    if (createdAt.getTime() > now) createdAt.setTime(createdAt.getTime() - DAY);
    ...
    return {
      ...
      lastMessageAt: new Date(Math.min(createdAt.getTime() + replyGap, now)),
    };
  });
...
/**
 * What identifies a demo ticket: its subject and its customer's address,
 * joined on a NUL, which neither contains, so the pair reads back one way. It
 * also names the Message-IDs (`hashKey`), so it must not change shape.
 */
function keyOf(row: { subject: string; customerEmail: string }): string {
  return `${row.subject}\u0000${row.customerEmail}`;
}

/** What one `seedShowcase` call did, for the script to report. */
export interface ShowcaseSeeded {
  /** Demo tickets deleted first — only ever non-zero with `reset`. */
  removed: number;
  created: number;
  /** Messages written onto the tickets created. */
  messages: number;
  /** Demo tickets found with no thread, and given one. */
  backfilledTickets: number;
  backfilledMessages: number;
}

/**
 * Put the showcase in the database.
 *
 * Plain: append every demo ticket not already there, and backfill a thread
 * onto any demo ticket that has none. Nothing already present is changed,
 * which is what makes it safe to run against a showcase somebody is using.
 *
 * `reset`: delete every demo ticket and insert the lot afresh, so each is back
 * to its seeded status, category, assignee and thread. **The delete and the
 * inserts are one transaction** — the nightly runs while a visitor may be
 * mid-demo, and between the two they would otherwise find an empty desk. The
 * array form, as the script always used: no step reads another's result, so an
 * interactive one would add only its 5s default timeout over a hundred inserts.
 *
 * Seeded tickets are spread across `ASSIGNABLE_USER` — the people a ticket may
 * be handed to — so none is ever filed under a demo visitor or the assistant,
 * whoever exists when it runs. Throws when nobody does: `db:seed` comes first.
 */
export async function seedShowcase(
  options: { reset?: boolean; now?: number } = {},
): Promise<ShowcaseSeeded> {
  const { reset = false, now = Date.now() } = options;

  const assignees = await prisma.user.findMany({
    where: ASSIGNABLE_USER,
    // name and email are what a support reply is signed with, not just the id
    // the ticket is assigned by.
  ...
  const rows = buildRows(assignees, now);
  ...
  const doomed = reset
    ? existing.filter((t) => demoKeys.has(keyOf(t))).map((t) => t.id)
    : [];
  // After a reset nothing demo-keyed survives, so every row is due.
  const alreadyThere = new Set(reset ? [] : existing.map(keyOf));
  ...
  // One transaction, so a failure part-way through leaves neither half a batch
  // behind for the next run to trip over nor a reset that deleted and stopped.
  // Messages and the activity trail cascade on ticket delete (see schema.prisma).
  await prisma.$transaction([
    prisma.ticket.deleteMany({ where: { id: { in: doomed } } }),
  ...
  return {
    ...
    messages: toCreate.reduce((n, row) => n + row.messages.length, 0),
    backfilledTickets: backfilled.tickets,
    backfilledMessages: backfilled.messages,
  };
}

/**
 * Demo tickets seeded before threads were written still have none, and leaving
 * them empty would mean the detail page shows its empty state for most of the
 * database. Only demo rows are touched — a ticket that arrived through the
 * webhook, or that someone made by hand, is not ours to put words into.
 */
async function backfillThreads(
  rows: TicketRow[],
  demoKeys: Set<string>,
  assigneesById: Map<string, Assignee>,
): Promise<{ tickets: number; messages: number }> {
  ...
  if (emptyDemoTickets.length === 0) return { tickets: 0, messages: 0 };

  const backfilled = await prisma.$transaction(
    emptyDemoTickets.map((ticket) => {
      const key = keyOf(ticket);
      // The row's own dates, not the freshly computed ones: time has moved on
  ...
  return {
    tickets: emptyDemoTickets.length,
    messages: backfilled.reduce((n, result) => n + result.count, 0),
  };
}
