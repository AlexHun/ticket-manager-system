 ...
 *
 * Beside `mode.ts`, and importing nothing but it — which itself imports
 * nothing — so nothing mocks either. `auth.ts` applies both rules, and they are
 * tested through it in `routes/users.test.ts`, the one file that loads it for
 * real. See `docs/adr/0022-a-demo-session-is-an-anonymous-agent.md`.
 */
import { isDemoModeEnabled } from "./mode";

/** Two hours from the start, not from the last request (R7). */
const DEMO_SESSION_MS = 2 * 60 * 60 * 1000;
...
export function demoSessionRefused(
  ...
  return Date.now() >= demoSessionEndsAt(new Date(session.createdAt)).getTime();
}
