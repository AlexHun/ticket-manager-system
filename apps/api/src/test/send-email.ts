 ...
 * shape, green. `jobs/send-email.test.ts` (#325) covers the worker and
 * `withholdEmail`, which both share `enqueueEmail`'s row builder, but not
 * `enqueueEmail` itself — that is the gap left, and it is that module's to close.
 */
