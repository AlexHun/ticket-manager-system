...
| `bun run test`        | both unit suites, `@ticket/api` then `@ticket/web`. ~5.5 min on Windows; `.husky/pre-push` runs only the API half |
...
| `bun run tokens`      | forecast vs. actual token spend per issue, from the local Claude Code transcripts plus one `gh issue list` call for titles and `forecast/S\|M\|L` labels — reads nothing in this repo and writes nothing anywhere. Without `gh` the actuals still print, the forecast columns read `-` and the title is blank. `CLAUDE_TRANSCRIPT_DIR` overrides which transcript directory it reads and `GH_ISSUES_FILE` overrides the issue listing with a JSON file; the join, the bands and the verdicts live in `apps/web/dev/usage.ts` and `issues.ts`, shared with the dev-tools Usage page so the two cannot disagree. Every **open** issue prints too, with its band and `-` for every figure, so what is forecast and not yet started reads beside what it cost; those rows are excluded from the percentiles and the accuracy figure. The last line totals what ran on `main` or with no branch, in turns **and** output tokens — it belongs to no issue and is in none of the rows above |
